...
pub fn loadWarRoomData(c: *spider.Ctx, workspace_id: i32) !model.WarRoomData {
    ...
    const pane_rows = try db.query(
        ...
        c.arena,
        \\SELECT wp.id, wp.role_name, COALESCE(a.name, '') AS agent_name, COALESCE(a.handle, '') AS agent_handle, wp.pane_state, COALESCE(wp.session_external_id, '') AS session_external_id, wp.context_state
